...
void main() {
  group('$ReorderableListView', () {
    ...
    group('in vertical mode', () {
      testWidgets('reorders its contents only when a drag finishes',
      ...
      testWidgets('properly reorders with a header',
          (WidgetTester tester) async {
      ...
      testWidgets('properly determines the vertical drop area extents',
        ...
        Element getContentElement() {
          final SingleChildScrollView listScrollView =
              tester.widget(find.byType(SingleChildScrollView));
          final Widget scrollContents = listScrollView.child;
          final Element contentElement = tester.element(find.byElementPredicate(
              (Element element) => element.widget == scrollContents));
      ...
      testWidgets('Uses the PrimaryScrollController when available',
          (WidgetTester tester) async {
        final ScrollController primary = ScrollController();
        final Widget reorderableList = ReorderableListView(
          children: const <Widget>[
            SizedBox(
                width: 100.0, height: 100.0, child: Text('C'), key: Key('C')),
            SizedBox(
                width: 100.0, height: 100.0, child: Text('B'), key: Key('B')),
            SizedBox(
                width: 100.0, height: 100.0, child: Text('A'), key: Key('A')),
          ],
          onReorder: (int oldIndex, int newIndex) {},
        );
      ...
      group('Accessibility (a11y/Semantics)', () {
        ...
        testWidgets('First item accessibility (a11y) actions work',
            (WidgetTester tester) async {
          final SemanticsHandle handle = tester.ensureSemantics();
          expect(listItems, orderedEquals(originalListItems));
        ...
        testWidgets('Middle item accessibility (a11y) actions work',
            (WidgetTester tester) async {
          final SemanticsHandle handle = tester.ensureSemantics();
          expect(listItems, orderedEquals(originalListItems));
        ...
        testWidgets('Last item accessibility (a11y) actions work',
            (WidgetTester tester) async {
          final SemanticsHandle handle = tester.ensureSemantics();
          expect(listItems, orderedEquals(originalListItems));
    ...
    testWidgets('ReorderableListView can be reversed',
        (WidgetTester tester) async {
      final Widget reorderableListView = ReorderableListView(
        children: const <Widget>[
          SizedBox(
    ...
    /*testWidgets('Reorderable List Item with default elevation when dragging',
        (WidgetTester tester) async {
      final Widget reorderableListViewWithDefaultElevation =
          ReorderableListView(
        children: const <Widget>[
          Card(
            key: Key('A'),
      ...
      await tester.pumpWidget(MaterialApp(
        home: reorderableListViewWithDefaultElevation,
      ));

      final Finder finder = find.byKey(const Key('A'));
      final Card item = tester.widget<Card>(find.byKey(const Key('A')));

      *//*await tester.longPress(finder);
      await tester.pumpAndSettle(const Duration(seconds: 1));*//*
      expect(item.elevation, null);
    });

    testWidgets('Reorderable List Item with custom elevation when dragging',
        (WidgetTester tester) async {
      final Widget reorderableListViewWithCustomElevation = ReorderableListView(
        children: const <Widget>[
          Card(
            key: Key('A'),
            child: SizedBox(
        ...
        reverse: true,
        onReorder: (int oldIndex, int newIndex) {},
        dragItemElevation: 2.0,
      );
      await tester.pumpWidget(MaterialApp(
        home: reorderableListViewWithCustomElevation,
      ));

      await tester.longPress(find.byKey(const Key('A')));
      await tester.pumpWidget(
          tester.widget<Card>(find.byKey(const Key('A'))), kLongPressTimeout);
      expect(tester.widget<Card>(find.byKey(const Key('A'))).elevation, 2.0);
    });*/
  });
}
